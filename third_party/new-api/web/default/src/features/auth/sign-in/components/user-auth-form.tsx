...
export function UserAuthForm({
  ...
  const routerSSOEnabled = Boolean(
    status?.router_sso_enabled ?? status?.data?.router_sso_enabled
  )
  const routerSSOLoginUrl = String(
    status?.router_sso_login_url || status?.data?.router_sso_login_url || '/sso/login'
  )
  ...
  function handleRouterSSOLogin() {
    const returnTo =
      typeof window !== 'undefined' ? window.location.href : redirectTo || '/'
    const separator = routerSSOLoginUrl.includes('?') ? '&' : '?'
    window.location.href = `${routerSSOLoginUrl}${separator}return_to=${encodeURIComponent(returnTo)}`
  }
    ...
    return (
      <form className={cn('grid gap-4', className)} {...props}>
        <Button
          ...
          onClick={handleRouterSSOLogin}
        >
          <LogIn />
          {t('Sign in with company SSO')}
        </Button>
      </form>
    )
  }
