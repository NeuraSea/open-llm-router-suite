...
func applyServerAddressFromEnv() {
	serverAddress := strings.TrimRight(strings.TrimSpace(os.Getenv("SERVER_ADDRESS")), "/")
	if serverAddress == "" {
		return
	}

	common.OptionMapRWMutex.RLock()
	currentOption := strings.TrimSpace(common.OptionMap["ServerAddress"])
	currentRuntime := strings.TrimSpace(system_setting.ServerAddress)
	common.OptionMapRWMutex.RUnlock()

	if currentOption != "" && currentOption != "http://localhost:3000" {
		return
	}
	if currentRuntime != "" && currentRuntime != "http://localhost:3000" {
		return
	}
	if err := updateOptionMap("ServerAddress", serverAddress); err != nil {
		common.SysLog("failed to apply SERVER_ADDRESS: " + err.Error())
	}
}
